...
import { getObjectByPlural, isGroupable } from "@/lib/metadata"
...
import { ViewSwitcher } from "@/components/platform/record/view-switcher"
...
export async function RecordContent({
  ...
  const groupables = object.fields
    .filter(isGroupable)
    .map((f) => ({ name: f.name, label: f.label }))
  const boardView = searchParams.view === "board" && groupables.length > 0

  // Board (kanban) mode: a switcher bar over the generic grouped board. Skips the
  // table fetch entirely.
  if (boardView) {
    return (
      <div>
    ...
    <div className="container-wrapper py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {groupables.length > 0 ? (
            <ViewSwitcher groupables={groupables} view="table" />
          ) : null}
          <RecordForm
